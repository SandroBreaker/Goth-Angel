...
const DataGrid: React.FC<{ title: string; columns: string[]; rows: any[] }> = ({ title, columns, rows }) => (
  <div className="mb-12 border border-neutral-900 bg-black/40 overflow-hidden">
    <div className="bg-neutral-900/50 p-4 border-b border-neutral-800 flex items-center gap-3">
      ...
      <h4 className="font-mono text-[9px] md:text-[10px] text-white tracking-[0.2em] uppercase font-bold">{title}</h4>
    </div>
    <div className="overflow-x-auto scrollbar-hide">
      <table className="w-full text-left font-mono text-[8px] md:text-[9px] tracking-wider uppercase min-w-[500px] md:min-w-0">
          ...
          {rows.map((row, i) => (
            <tr key={i} className="hover:bg-[#FF007F]/5 transition-colors">
              {Object.values(row).map((val: any, j) => (
                <td key={j} className="p-4 text-neutral-300 border-r border-neutral-900 last:border-0 leading-relaxed">{val}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
const AnalysisBlock: React.FC<{ title: string; children: React.ReactNode; icon: React.ReactNode }> = ({ title, children, icon }) => (
  <div className="mb-12 md:mb-16">
    <div className="flex items-center gap-4 mb-6">
      ...
      <h3 className="font-serif-classic text-sm md:text-lg text-white tracking-[0.2em] uppercase">{title}</h3>
      <div className="h-px flex-grow bg-neutral-900" />
    </div>
    <div className="font-mono text-[10px] md:text-[11px] text-neutral-400 leading-relaxed uppercase space-y-4 max-w-4xl italic border-l border-[#FF007F]/20 pl-4 md:pl-6">
      {children}
    </div>
  </div>
);
...
export const TheVault: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto py-16 md:py-24 px-6 pb-48">
      <section className="mb-12 md:mb-16 grid grid-cols-1 lg:grid-cols-3 gap-10 lg:gap-12 items-end border-b border-neutral-900 pb-12 md:pb-16">
        <div className="lg:col-span-2">
          <div className="flex items-center gap-3 mb-6">
            <Fingerprint size={24} className="text-[#FF007F] opacity-50" />
            <span className="font-mono text-[9px] md:text-[10px] text-neutral-600 tracking-[0.4em] md:tracking-[0.5em] uppercase">Archive Subject #110196</span>
          </div>
          <h2 className="font-serif-classic text-3xl md:text-5xl lg:text-7xl text-white tracking-tighter mb-4 drop-shadow-[0_0_20px_rgba(255,255,255,0.1)] leading-tight">
            {dossierData.subject.realName.toUpperCase()}
          </h2>
          <div className="flex flex-wrap gap-3 md:gap-4 font-mono text-[8px] md:text-[9px] tracking-[0.2em] md:tracking-[0.3em] uppercase">
        ...
        <div className="space-y-3 bg-neutral-950 p-5 md:p-6 border border-neutral-900">
          <div className="flex justify-between items-center text-[9px] md:text-[10px] font-mono tracking-widest uppercase">
            ...
            <span className="text-neutral-200 text-right ml-4">{dossierData.subject.birth}</span>
          </div>
          <div className="flex justify-between items-center text-[9px] md:text-[10px] font-mono tracking-widest uppercase">
            <span className="text-neutral-600">Terminal</span>
            <span className="text-neutral-200 text-right ml-4">{dossierData.subject.death}</span>
          </div>
          <div className="h-px bg-neutral-900 my-2" />
          <p className="text-[8px] font-mono text-[#FF007F] tracking-[0.1em] leading-relaxed uppercase opacity-80">
            CAUSE: {dossierData.subject.cause}
          </p>
        </div>
      </section>

      <div className="flex gap-2 md:gap-4 mb-12 md:mb-16 border-b border-neutral-900 overflow-x-auto scrollbar-hide">
        <button 
          onClick={() => setActiveTab('dossier')}
          className={`pb-4 px-4 md:px-6 font-mono text-[9px] md:text-[10px] tracking-[0.2em] md:tracking-[0.3em] uppercase transition-all relative shrink-0 ${activeTab === 'dossier' ? 'text-[#FF007F]' : 'text-neutral-600 hover:text-neutral-400'}`}
        >
        ...
        <button 
          ...
          className={`pb-4 px-4 md:px-6 font-mono text-[9px] md:text-[10px] tracking-[0.2em] md:tracking-[0.3em] uppercase transition-all relative shrink-0 ${activeTab === 'analysis' ? 'text-[#7000FF]' : 'text-neutral-600 hover:text-neutral-400'}`}
        >
        ...
        {activeTab === 'dossier' ? (
          ...
          >
            {dossierData.sections.map((section) => (
              <div key={section.id} className="border border-neutral-900 bg-[#080808]/40 p-6 md:p-8 group">
                <div className="flex items-center gap-4 mb-6 md:mb-8">
                  <div className="text-[#7000FF]">{React.cloneElement(section.icon as any, { size: 16 })}</div>
                  <h3 className="font-serif-classic text-xs md:text-sm tracking-[0.2em] text-white uppercase">{section.title}</h3>
                </div>
                <div className="space-y-6">
                  {section.items.map((item: any, idx: number) => (
                    <div key={idx} className="flex gap-4">
                      <div className="w-1 h-1 bg-[#FF007F]/40 rounded-full mt-2 shrink-0" />
                      <div>
                        <span className="font-mono text-[8px] text-[#7000FF] uppercase tracking-widest block mb-1">{item.label}</span>
                        <p className="font-mono text-[9px] md:text-[10px] text-neutral-400 leading-relaxed uppercase">{item.content}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <MotionDiv 
            ...
            exit={{ opacity: 0, x: -20 }}
          >
            <AnalysisBlock title="A VOZ DE UMA GERAÇÃO" icon={<BookOpen size={16} />}>
            ...
            <AnalysisBlock title={dossierData.analysis.architecture.title} icon={<Zap size={16} />}>
            ...
            <AnalysisBlock title="CONCLUSÃO DO ARQUIVO" icon={<Archive size={16} />}>
      ...
      <div className="mt-20 md:mt-24 p-8 md:p-12 border border-dashed border-neutral-900 text-center relative group">
        ...
        <p className="text-[9px] md:text-[10px] font-mono text-neutral-600 uppercase tracking-[0.4em] mb-2">
          Heritage Protected. Integridade de catálogo restaurada.
        </p>
        <p className="text-[7px] md:text-[8px] font-mono text-neutral-800 uppercase tracking-[0.2em] px-4">
