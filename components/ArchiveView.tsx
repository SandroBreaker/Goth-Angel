                  ...
                  <div className="relative z-10 max-w-7xl w-full px-8 flex flex-col md:flex-row items-center gap-12 lg:gap-24">
                    ...
                    >
                      ...
                      <img 
                        ...
                        className={`w-64 h-64 lg:w-[380px] lg:h-[380px] object-cover border-2 border-neutral-800 shadow-[0_0_50px_rgba(0,0,0,0.5)] relative z-10 transition-transform duration-1000 ${isFeaturedPlaying ? 'animate-[spin_30s_linear_infinite]' : ''}`}
                      />
                    </MotionDiv>
                      ...
                      <MotionDiv initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}>
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start gap-4">
                          ...
                          <button 
                            ...
                            className={`group relative inline-flex items-center gap-4 px-8 py-4 transition-all border-2 overflow-hidden ${isFeaturedPlayable ? 'bg-white text-black border-white hover:bg-black hover:text-white' : 'bg-neutral-900 text-neutral-600 border-neutral-800 cursor-not-allowed'}`}
                          >
                            <span className="relative z-10 font-mono text-[11px] font-bold tracking-[0.2em] uppercase">
              ...
              {categories.length > 0 ? (
                categories.map((cat, idx) => (
                  <div key={cat.id} className="relative">
                    <div className="px-8 mb-8 flex items-end justify-between border-b border-neutral-900 pb-5 mx-8">
                      <div>
                        <div className="flex items-center gap-3 text-[#7000FF] mb-2">
                          {cat.icon}
                          <span className="font-mono text-[10px] font-bold tracking-[0.3em] uppercase">Era {idx + 1}</span>
                        </div>
                        <h3 className="font-serif-classic text-2xl md:text-3xl text-white tracking-widest uppercase">{cat.title}</h3>
                      </div>
                    </div>
            ...
            <div className="mt-32 px-8 pt-16 border-t border-neutral-800">
              ...
              {hasMore && !loading && (
                <div className="mt-20 flex justify-center">
                  <button 
                    onClick={onLoadMore}
                    className="group flex flex-col items-center gap-4 text-neutral-500 hover:text-[#FF007F] transition-all"
                  >
                    <span className="font-mono text-[11px] font-bold tracking-[0.4em] uppercase">More Artifacts</span>
                    <div className="p-4 border border-neutral-900 rounded-full group-hover:border-[#FF007F]/40 animate-bounce transition-colors">
                      <ArrowDown size={18} />
                    </div>
                  </button>
                </div>
