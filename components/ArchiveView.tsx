        ...
        {loading && isInitialLoad ? (
          ...
          >
            <div className="relative mb-8">
              <Loader2 className="w-16 h-16 text-[#FF007F] animate-spin opacity-20" />
              <div className="absolute inset-0 flex items-center justify-center">
                <Cpu className="w-6 h-6 text-[#FF007F] animate-pulse" />
              </div>
              <div className="absolute -inset-4 border border-[#FF007F]/10 rounded-full animate-ping duration-[3000ms]"></div>
            </div>
            <div className="flex flex-col items-center gap-2">
              <span className="font-mono text-[10px] text-[#FF007F] tracking-[0.8em] uppercase animate-pulse">Scanning Archive</span>
              <span className="font-mono text-[7px] text-neutral-700 tracking-[0.4em] uppercase">Reconstructing Digital Memory...</span>
            </div>
          </MotionDiv>
        ) : (
          <MotionDiv 
                ...
                categories.map((cat, idx) => (
                  <div key={cat.id} className="relative">
                    ...
                    <MotionDiv 
                      ...
                      className="flex gap-4 overflow-x-auto px-6 pb-8 snap-x scrollbar-hide"
                    >
                      {cat.data.map((song) => (
                        <MotionDiv key={song.id} variants={itemVariants} className="w-48 lg:w-64 shrink-0 snap-start">
